...
public class Chuck : MonoBehaviour
{
    ...
    public GameObject Charles; // Player 1 (for when 2 player mode is selected)
    ...
    void Start()
    {
        ...
        stageHeight = PlayerPrefs.GetFloat("StageHeight", 0);
    }
    ...
    void Update()
    {
        ...
        if (!otherPlayer && !bossBattle)
            mainCam.transform.position = new Vector3(this.transform.position.x + 20, stageHeight, -10);
    }
    ...
    private void damage()
    {
        ...
        if (health <= 0)
        {
            lives.GetComponent<Vitals>().p2die();
